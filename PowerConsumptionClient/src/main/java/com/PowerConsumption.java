...
public class PowerConsumption extends DBHandler{
	...
	public String insertConsumption(String currentReading, String date, String type, String readerID, String userID) {
		...
		try {
			...
			String[] aa = PowerConsumption.getActNumberById(userID);
			...
			if(noUnit[0]==-1) {
				String msg = "Error while connecting to the database!";
				return "{\"status\":\"error\", \"data\":\""+msg+".\"}";
				
			}
			else if(noUnit[0]==-2) {
				String msg = "Error while reading previous reading";
				return "{\"status\":\"error\", \"data\":\""+msg+".\"}";
			}
			else if(noUnit[1]<0){
				String msg =  "Incorrect input: Number of units in Current reading cannot be less than previous reading ";
				return "{\"status\":\"error\", \"data\":\""+msg+".\"}";
			}
		...
		{
			output = "{\"status\":\"error\", \"data\":\"Error:"+e.getMessage()+".\"}";
	...
	public String readConsumption() {
		...
		try {
			...
			while (rs.next()) {
				...
				output += "<tr><td><input id='hidItemIDUpdate' name='hidItemIDUpdate' type='hidden' value='" + recordID + "'>"
						+ recordID + "</td>";
				output += "<td>" + actno + "</td>";
